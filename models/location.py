...
class Location:
    ...
    def get_distance_to(self, other_city: str):
        if other_city not in self.distances[self.name]:
            raise ValueError(f"No direct route from {self.name} to {other_city}")
        return self.distances[self.name][other_city]
